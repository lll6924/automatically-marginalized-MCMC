...
model_code = """
    ...
    model {
        ...
        y ~ beta_binomial(K, phi * kappa, (1 - phi) * kappa);                        // likelihood
    }
    generated quantities {
        vector<lower=0, upper=1>[N] theta;    
        theta = to_vector(beta_rng(phi * kappa + to_vector(y), (1 - phi) * kappa + to_vector(K) - to_vector(y)));
    }
"""
...
def main(dataset, seed):
    ...
    fit = sm.sampling(data=data, chains=1, warmup=10000, iter=110000, seed=seed, verbose = True)
    time2 = time()
    ...
    result_path = 'result/StanHPP_m/{}/'.format(dataset)
